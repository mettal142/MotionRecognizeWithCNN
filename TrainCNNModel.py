...
def build_CNN_classifier(x):
    ...
    W_fc1= tf.Variable(tf.truncated_normal(shape=[20*6*128,1024],stddev=5e-2))
    b_fc1= tf.Variable(tf.constant(0.1,shape=[1024]))

    h_conv5_flat = tf.reshape(h_conv5,[-1,20*6*128])
...
with tf.Session() as sess:
    ...
    for i in range(80):
        ...
        sess.run([train_step],feed_dict={x:bx,y:by,keep_prob:1.0})
        if train_accuracy==1:
            break
